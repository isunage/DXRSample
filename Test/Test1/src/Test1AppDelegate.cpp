...
void test::Test1AppDelegate::InitAccelerationStructures()
{
	...
	if (m_Context->SupportDXR()) {
		...
		auto srvUavHandle = CD3DX12_CPU_DESCRIPTOR_HANDLE(m_SrvUavHeap->GetCPUDescriptorHandleForHeapStart());

		D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc = {};
		uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
		m_Context->GetDevice()->CreateUnorderedAccessView(m_RtOutput.Get(), nullptr, &uavDesc, srvUavHandle);
...
void test::Test1AppDelegate::InitShaderTable()
{
	...
	m_ShaderTableEntrySize   = rtlib::getDX12AlignedSize(m_ShaderTableEntrySize, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
	uint32_t shaderTableSize = m_ShaderTableEntrySize * 3;

	rtlib::ThrowIfFailed(m_Context->GetDevice()->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(shaderTableSize, D3D12_RESOURCE_FLAG_NONE),
		D3D12_RESOURCE_STATE_GENERIC_READ,nullptr,
		IID_PPV_ARGS(&m_ShaderTable)
	));
	
	UINT8* pData;
	rtlib::ThrowIfFailed(m_ShaderTable->Map(0, nullptr, (void**)&pData));

	rtlib::ComPtr<ID3D12StateObjectProperties> stateObjectProperties;
	rtlib::ThrowIfFailed(m_StateObject.As(&stateObjectProperties));

	std::memcpy(pData + 0 * m_ShaderTableEntrySize, stateObjectProperties->GetShaderIdentifier(L"rayGen")   , D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
	std::memcpy(pData + 1 * m_ShaderTableEntrySize, stateObjectProperties->GetShaderIdentifier(L"miss")     , D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
	std::memcpy(pData + 2 * m_ShaderTableEntrySize, stateObjectProperties->GetShaderIdentifier(L"hitGroup") , D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
